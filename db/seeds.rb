def groups()
    UserGroup::Group.create(name: "admin", description: "Members of this group have admin permissions", is_locked: true)
    UserGroup::Group.create(name: "registered", description: "Every user account is a member of this group.", is_locked: true)
end
...
def add_regular_user(email,first_name,second_name,locale)
    user = UserGroup::User.create(:email => email, :password => "password", :password_confirmation => "password", :locale => locale, :first_name => first_name, :second_name => second_name) 
