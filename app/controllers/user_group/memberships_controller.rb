...
module UserGroup
  class MembershipsController < ApplicationController
    ...
    before_action :admin_users, only: %i[create approve]
    # :can_modify (through can_modify_base) also sets @user (found by id ONLY)
    before_action :can_modify?, only: [:destroy]
    before_action :collection_mgr_users, only: %i[approve_read remove_read view_read_request]
    # remote true sends requests in js
    respond_to :html, :js

    def create
      @user = user(params[:membership][:user_id])
      group_id = group_id(params[:membership][:group_id])
      ...
      if @user.nil?
        ...
        render 'users/edit' and return if membership.errors.count.positive?
    ...
    def destroy
      ...
      if action.nil?
        ...
        render 'users/edit' and return if action.errors.count.positive?
    ...
    def user(user_id_or_name)
      not_positive_integer?(user_id_or_name) ? User.find_by_email(user_id_or_name) : User.find_by_id(user_id_or_name)
    end

    def group_id(group_id_or_name)
      not_positive_integer?(group_id_or_name) ? group_id_from_group_name(group_id_or_name) : group_id_or_name
    end

    def group(group_id_or_name)
      Group.find(not_positive_integer?(group_id_or_name) ? group_id_from_group_name(group_id_or_name) : group_id_or_name)
    end

    def group_id_from_group_name(group_name)
      group = Group.find_by_name(group_name.downcase)
      group&.id
    end

    def not_positive_integer?(string)
      true unless string =~ /^[0-9]+$/
    end
  end
end
