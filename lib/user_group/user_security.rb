...
module UserGroup
  module UserSecurity
    ...
    def is_admin?
      ...
      true if !group.nil? && member?(group.id)
    end
    ...
    def member?(group_id)
      membership = memberships.find_by_group_id(group_id)
      true if !membership.nil? && membership.approved?
    end
    ...
    def token_expired?
      if authentication_token.blank? || ((SETTING_PROFILE_TOKEN_EXPIRY_DAYS != 0) && (Date.today > token_age_allowed))
        return true
      end
    ...
    def token_age_allowed
      token_creation_date.to_date + SETTING_PROFILE_TOKEN_EXPIRY_DAYS
    end
