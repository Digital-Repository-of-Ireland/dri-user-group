...
module Blacklight::AccessControls
  module Ability
    ...
    included do
      ...
      self.ability_logic = %i[create_permissions edit_permissions read_permissions custom_permissions]
      # 383 Addition
      self.ability_logic += %i[manager_permissions search_permissions]
    end

    def self.user_class
      Blacklight::AccessControls.config[:user_model] ? Blacklight::AccessControls.config[:user_model].constantize : ::User
    end
