    ...
    def generate_permission_filters(permission_types=["discover", "manager", "edit", "read"])
      ...
      permission_types.each do |type|
        ...
        if type == "manager" || type == "edit"
          permission_query = "_query_:\"{!join from=id to=ancestor_id_tesim}" + permission_query + "\" OR " +
                               "("+permission_query+")"
        elsif type == "discover"
         ...
         "_query_:\"{!join from=id to=ancestor_id_tesim}" + ActiveFedora::SolrService.solr_name("private_metadata", Hydra::Datastream::RightsMetadata.integer_indexer) + ":0 OR " +
          ...
          permission_query = "((" + escape_filter(ActiveFedora::SolrService.solr_name("#{type}_access_inherit", Hydra::Datastream::RightsMetadata.indexer), "true") + " AND " +
                             "_query_:\"{!join from=id to=ancestor_id_tesim}" + permission_query + "\" ) OR " + 
